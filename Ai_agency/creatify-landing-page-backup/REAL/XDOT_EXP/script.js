...
/*
    ...
    // Removed FeaturedWorkAnimations class - all project card code removed
    /*
    ...
    */
    ...
    // Removed Featured Work Animations initialization - all project card code removed

    // ===== FEATURED WORK SECTION 3 ANIMATIONS =====
    // Removed Featured Work Section 3 animations - all project card code removed
    /*
    ...
    */
    ...
    function initAnnouncementBar() {
        ...
        // Function to hide the bar
        function hideBar() {
            // Add class to animate bar sliding up
            bar.classList.add('bar-hidden');

            // Set padding-top of body to 0
            body.style.paddingTop = '0';

            // Adjust header margin-top to 0 when bar is hidden
        ...
        // Check if user has already hidden the bar
        if (localStorage.getItem('announcementBarHidden') === 'true') {
            hideBar();
        } else {
            // If not hidden, make sure the body padding is correct
            // We do this in JS to get the bar's actual height
            body.style.paddingTop = `${bar.offsetHeight}px`;

            // Also set header margin-top to account for the bar
            const header = document.querySelector('.header');
            if (header) {
        ...
        // Adjust padding if window is resized
        window.addEventListener('resize', () => {
            if (localStorage.getItem('announcementBarHidden') !== 'true') {
                body.style.paddingTop = `${bar.offsetHeight}px`;

                // Also adjust header margin-top on resize
                const header = document.querySelector('.header');
                if (header) {
                    header.style.marginTop = `${bar.offsetHeight}px`;
                }
    ...
    // Initialize popup system for index.html
    initPopupEvents();
    initIndexPopup();
    ...
    const projectData = {
        ...
        "ai-catalog-1": {
            ...
            galleryImages: [
                { src: "assets/greenPower.png", caption: "Contextual Shot" },
                { src: "assets/earring.jpeg", caption: "Detail Shot" },
                { src: "assets/sunpower.png", caption: "Angle 3" },
                { src: "assets/water_bottle.jpeg", caption: "Angle 4" }
            ],
        ...
        "ai-catalog-2": {
            ...
            beforeImage: "assets/icecream.jpeg",
            afterImage: "assets/shine.jpeg",
            galleryImages: [
                { src: "assets/princles.jpeg", caption: "Product Display" },
                { src: "assets/salon2.png", caption: "Lifestyle Shot" },
                { src: "assets/sunsscreen.jpeg", caption: "Detail View" }
            ],
            description: "A fashion brand needed to showcase their new collection with professional product photography. Using AI-powered image enhancement, we transformed their mobile photos into high-end fashion catalog images that perfectly captured the brand's aesthetic and increased their online sales by 150%.",
            tags: ["AI Enhancement", "Fashion Photography", "Brand Consistency", "E-commerce"],
            demoUrl: null
        }
    };
    ...
    // ===== POPUP SYSTEM FOR INDEX.HTML =====

    // Popup content for index.html (Free Sample)
    ...
    function initIndexPopup() {
        ...
        // Only show first popup if not previously shown
        if (!firstPopupShown) {
            // Show first popup after 5 seconds (temporarily reduced for testing)
            setTimeout(() => {
                if (!localStorage.getItem('firstPopupShown')) {
                    showPopup(indexPopupContent);
    ...
    // ===== END: POPUP SYSTEM FOR INDEX.HTML =====
