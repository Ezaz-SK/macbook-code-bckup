document.addEventListener('DOMContentLoaded', function() {
    ...
    function setActiveNavigation() {
        ...
        if (currentPath.includes('index.html') || currentPath === '/' || currentPath === '') {
            ...
            // Works page
            const worksLink = document.querySelector('.nav-link[href="work.html"], .nav-link[href="#"]');
    ...
    if (heroSection && mediaElementsLength > 0) {
    ...
    if (manifestoSection) {
        ...
        // Removed rotating/floating animations for cleaner performance
        // Manifesto section elements now use static positioning
    ...
    // --- Advanced Real-time Scroll Animation System (Removed) ---
    // Removed section stacking animations for better performance and cleaner scroll behavior
    // Hero and manifesto sections now use standard static positioning without scroll-based animations
...
/*
    ...
    try {
        ...
        console.error('Error initializing services system:', error);
    }
*/
...
/*
    ...
    // --- Featured Work Section Animations ---
    ...
    if (featuredWorkSection) {
        // Ensure section header is visible immediately
        const sectionHeader = featuredWorkSection.querySelector('.text-center.mb-20');
        if (sectionHeader) {
        ...
        // Also ensure all child elements of section header are visible
        const headerElements = sectionHeader.querySelectorAll('h2, p');
        ...
        // Make project cards visible immediately (fix for visibility issue)
        projectCards.forEach((card) => {
            card.classList.add('animate-in');
        });

        // Fallback: Ensure cards are visible after 2 seconds if animation hasn't triggered
        setTimeout(() => {
            projectCards.forEach((card) => {
                if (!card.classList.contains('animate-in')) {
                    card.classList.add('animate-in');
                    console.log('Applied fallback animation to project card');
                }
            });
        }, 2000);

        // Create intersection observer for enhanced animations
        const featuredWorkObserver = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    ...
                    projectCards.forEach((card, index) => {
                        ...
                        setTimeout(() => {
                            ...
                            card.style.animation = `projectFloat 8s ease-in-out infinite`;
                            card.style.animationDelay = `${index * 0.5}s`;

                            // Add subtle random effects to some cards
                            if (Math.random() > 0.6) {
                                card.addEventListener('mouseenter', () => {
                                    card.style.transform += ' rotateY(2deg)';
                                    card.style.transition = 'transform 0.4s ease';
                                });

                                card.addEventListener('mouseleave', () => {
                                    card.style.transform = card.style.transform.replace(' rotateY(2deg)', '');
                                });
                            }
                        }, delay);
                    });

                    // Animate CTA section after project cards
                    setTimeout(() => {
                        if (featuredCTA) {
                            featuredCTA.style.opacity = '0';
                            featuredCTA.style.transform = 'translateY(30px) scale(0.95)';
                            featuredCTA.style.transition = 'all 1.2s cubic-bezier(0.16, 1, 0.3, 1)';

                            setTimeout(() => {
                                featuredCTA.style.opacity = '1';
                                featuredCTA.style.transform = 'translateY(0) scale(1)';

                                // Add glow animation to CTA
                                featuredCTA.style.animation = 'ctaGlow 3s ease-in-out infinite';
                            }, 300);
                        }
                    }, projectCards.length * 200 + 500);

                    // Unobserve after animation to prevent re-triggering
                    featuredWorkObserver.unobserve(entry.target);
                }
            });
        }, {
            threshold: 0.1,
        ...
        featuredWorkObserver.observe(featuredWorkSection);

        // Enhanced mouse tracking for project cards
        ...
        function animateFeaturedElements() {
            const rect = featuredWorkSection.getBoundingClientRect();
            ...
            if (isVisible) {
                ...
                projectCards.forEach((card, index) => {
                    if (card.classList.contains('animate-in')) {
                        ...
                        const rotateSpeed = 1 + (index * 0.3);

                        // Create subtle floating motion
                        const time = Date.now() * 0.001;
                        const floatY = Math.sin(time + index) * 1;
                        const floatX = Math.cos(time * 0.8 + index) * 0.5;

                        // Combine mouse parallax with floating animation
                        const translateX = (x * speed) + floatX;
                        const translateY = (y * speed) + floatY;
                        const rotateZ = (x * rotateSpeed) + (Math.sin(time * 0.7 + index) * 0.5);
            ...
            requestAnimationFrame(animateFeaturedElements);
        }
        ...
        projectCards.forEach(card => {
            ...
            card.addEventListener('mouseleave', function() {
                ...
                if (this.glowEffect) {
                    ...
                    setTimeout(() => {
                        if (this.glowEffect && this.glowEffect.parentNode) {
    ...
    // Add CSS for project-specific animations
    const featuredWorkStyles = document.createElement('style');
    featuredWorkStyles.textContent = `
        ...
        .project-card.animate-in {
            animation: projectCardReveal 1.2s cubic-bezier(0.16, 1, 0.3, 1) forwards;
        }
        ...
        .project-card:hover .metric-value {
            ...
            animation: featuredGradientShift 2s ease-in-out infinite;
        }
    ...
    `;
    document.head.appendChild(featuredWorkStyles);

    // ===== COMPREHENSIVE FEATURED WORK ANIMATIONS CLASS =====
    class FeaturedWorkAnimations {
        constructor() {
            this.featuredWorkSection3 = document.querySelector('.featured-work-section-3');
            this.projectCards3 = document.querySelectorAll('.project-card-3');
            this.featuredCTA3 = document.querySelector('.featured-cta-3');
        ...
        init() {
            if (!this.featuredWorkSection3) {
                console.warn('Featured Work Section 3 not found');
                return;
            }

            if (this.isInitialized) {
                console.warn('FeaturedWorkAnimations already initialized');
                return;
            }
            ...
            try {
                ...
                this.setupCTAButtonInteractions();
                this.setupMobileTouchSupport();
                this.setupKeyboardNavigation();
                this.setupReducedMotionSupport();
                this.setupCustomCursorIntegration();
                this.setupPerformanceOptimizations();
                this.setupDebugging();

                this.isInitialized = true;
                console.log('✅ FeaturedWorkAnimations initialized successfully');

                // Dispatch initialization event
                document.dispatchEvent(new CustomEvent('featuredWorkAnimations:initialized', {
                    detail: { section: this.featuredWorkSection3 }
                }));

            } catch (error) {
                console.error('❌ Error initializing FeaturedWorkAnimations:', error);
                this.handleInitializationError(error);
            }
        }

        setupIntersectionObserver() {
            // Create intersection observer for section 3 animations
            ...
            observer.observe(this.featuredWorkSection3);
            this.registerCleanupTask(() => observer.disconnect());
        }

        animateProjectCards() {
            // Animate project cards with staggered reveal
            this.projectCards3.forEach((card, index) => {
                const delay = index * (200 + Math.random() * 100); // 200-300ms random delays

                setTimeout(() => {
                    card.classList.add('animate-in');

                    // Add floating animation to cards
                    card.style.animation = `section3Float 10s ease-in-out infinite`;
                    card.style.animationDelay = `${index * 0.7}s`;

                    // Add interactive hover effects
                    this.addProjectCardHoverEffects(card);

                    // Dispatch card animation event
                    document.dispatchEvent(new CustomEvent('featuredWorkAnimations:cardAnimated', {
                        detail: { card, index }
                    }));
        ...
        animateCTASection() {
            // Animate CTA section after project cards
            setTimeout(() => {
                if (this.featuredCTA3) {
                    const ctaContainer = this.featuredCTA3.querySelector('.cta-container-3');
                    if (ctaContainer) {
                        ctaContainer.style.opacity = '0';
                        ctaContainer.style.transform = 'translateY(40px) scale(0.95)';
                        ctaContainer.style.transition = 'all 1.5s cubic-bezier(0.16, 1, 0.3, 1)';

                        setTimeout(() => {
                            ctaContainer.style.opacity = '1';
                            ctaContainer.style.transform = 'translateY(0) scale(1)';

                            // Add continuous glow animation to CTA
                            ctaContainer.style.animation = 'section3Glow 4s ease-in-out infinite';

                            // Dispatch CTA animation event
                            document.dispatchEvent(new CustomEvent('featuredWorkAnimations:ctaAnimated', {
                                detail: { cta: ctaContainer }
                            }));
                        }, 400);
                    }
                }
            }, this.projectCards3.length * 250 + 600);
        }

        setupMouseTracking() {
            // Enhanced mouse tracking for project cards 3
        ...
        startAnimationLoop() {
            const animateFeaturedElements3 = () => {
                const rect = this.featuredWorkSection3.getBoundingClientRect();
                const isVisible = rect.top < window.innerHeight && rect.bottom > 0;
                ...
                if (isVisible && !window.matchMedia('(prefers-reduced-motion: reduce)').matches) {
                    ...
                    this.projectCards3.forEach((card, index) => {
                        if (card.classList.contains('animate-in')) {
                            this.applyParallaxEffect(card, index, x, y);
                        }
                    });
                }

                this.animationId = requestAnimationFrame(animateFeaturedElements3);
            };

            animateFeaturedElements3();
        }

        applyParallaxEffect(card, index, mouseX, mouseY) {
            const speed = 1.5 + (index * 0.8);
            const rotateSpeed = 0.8 + (index * 0.4);

            // Create subtle floating motion
            const time = Date.now() * 0.0008;
            const floatY = Math.sin(time + index) * 2;
            const floatX = Math.cos(time * 0.9 + index) * 1;

            // Combine mouse parallax with floating animation
            const translateX = (mouseX * speed) + floatX;
            const translateY = (mouseY * speed) + floatY;
            const rotateZ = (mouseX * rotateSpeed) + (Math.sin(time * 0.8 + index) * 0.8);
        ...
        setupProjectCardInteractions() {
            this.projectCards3.forEach(card => {
                // Add click ripple effect
                this.addClickRippleEffect(card);

                // Add feature list animations
                this.addFeatureListAnimations(card);

                // Add project metrics animations
                this.addProjectMetricsAnimations(card);

                // Add intersection observer for individual card animations
                this.addIndividualCardObserver(card);

                // Add continuous subtle animations
                this.addContinuousAnimations(card);
            });
        }

        addProjectCardHoverEffects(card) {
            const mouseEnterHandler = () => {
                if (window.matchMedia('(prefers-reduced-motion: reduce)').matches) return;

                card.style.transform += ' rotateY(3deg) rotateX(-1deg)';
                card.style.transition = 'transform 0.5s ease';

                // Add dynamic glow effect
                this.addGlowEffect(card);

                // Update custom cursor state
                this.updateCustomCursor(card, 'hover');
            };

            const mouseLeaveHandler = () => {
                card.style.transform = card.style.transform.replace(/ rotateY\(3deg\) rotateX\(-1deg\)/g, '');
                this.removeGlowEffect(card);
                this.updateCustomCursor(card, 'normal');
            };

            card.addEventListener('mouseenter', mouseEnterHandler);
            card.addEventListener('mouseleave', mouseLeaveHandler);

            this.registerEventListener(card, 'mouseenter', mouseEnterHandler);
            this.registerEventListener(card, 'mouseleave', mouseLeaveHandler);
        }
        ...
        addGlowEffect(card) {
            ...
            glow.style.cssText = `
                ...
                right: -3px;
                bottom: -3px;
                background: linear-gradient(135deg, rgba(34, 197, 94, 0.4), rgba(59, 130, 246, 0.4), rgba(168, 85, 247, 0.4));
                border-radius: 35px;
                z-index: -1;
                opacity: 0;
            ...
            // Store glow reference for removal
            card.glowEffect3 = glow;
        }

        removeGlowEffect(card) {
            if (card.glowEffect3) {
                card.glowEffect3.style.opacity = '0';
                setTimeout(() => {
                    if (card.glowEffect3 && card.glowEffect3.parentNode) {
                        card.glowEffect3.remove();
                    }
                }, 400);
            }
            card.style.zIndex = '';
        ...
        addClickRippleEffect(card) {
            const clickHandler = (e) => {
                ...
                ripple.style.cssText = `
                    ...
                    border-radius: 50%;
                    transform: scale(0);
                    animation: section3Ripple 1s ease-out;
                    pointer-events: none;
                    z-index: 20;
                `;
                ...
                // Update custom cursor state
                this.updateCustomCursor(card, 'click');
            };

            card.addEventListener('click', clickHandler);
            this.registerEventListener(card, 'click', clickHandler);
        }

        addFeatureListAnimations(card) {
            const features = card.querySelectorAll('.project-features-3 li');
        ...
        addProjectMetricsAnimations(card) {
            const metrics = card.querySelectorAll('.metric');

            // Initial setup - hide metrics
            metrics.forEach(metric => {
                ...
                if (metricValue) {
                    ...
                    metricValue.style.transform = 'translateY(20px) scale(0.8)';
                    metricValue.style.transition = 'all 0.6s cubic-bezier(0.16, 1, 0.3, 1)';
                }

                if (metricLabel) {
                    metricLabel.style.opacity = '0.7';
                    metricLabel.style.transform = 'translateY(10px)';
                    metricLabel.style.transition = 'all 0.4s ease';
                }
            });

            // Animate metrics on card hover
            const mouseEnterHandler = () => {
                if (window.matchMedia('(prefers-reduced-motion: reduce)').matches) return;
                ...
                metrics.forEach((metric, index) => {
                    ...
                    setTimeout(() => {
                        if (metricValue) {
                            ...
                            metricValue.style.transform = 'translateY(0) scale(1)';

                            // Add subtle glow effect to metric values
                            metricValue.style.filter = 'drop-shadow(0 0 10px rgba(34, 197, 94, 0.3))';
                        }

                        if (metricLabel) {
                            metricLabel.style.opacity = '1';
                            metricLabel.style.transform = 'translateY(0)';
                        }
                    }, index * 150);
                });
            };
            ...
            // Add intersection observer for scroll-triggered metrics animation
            const metricsObserver = new IntersectionObserver((entries) => {
                entries.forEach(entry => {
                    if (entry.isIntersecting && entry.target === card) {
                        // Animate metrics when card comes into view
                        this.animateMetricsOnScroll(metrics);
            ...
            this.registerCleanupTask(() => metricsObserver.disconnect());
        }

        animateMetricsOnScroll(metrics) {
            // Animate metrics with staggered entrance on scroll
            metrics.forEach((metric, index) => {
                const metricValue = metric.querySelector('.metric-value');
                const metricLabel = metric.querySelector('.metric-label');
                ...
                setTimeout(() => {
                    ...
                    this.animateMetricValue(metricValue);

                }, index * 200);
            });
        }

        animateMetricValue(metricValue) {
            ...
            if (numberMatch) {
                ...
                // Only animate if it's a reasonable number for counting
                const numericValue = parseInt(number.replace(/,/g, ''));
                if (numericValue > 0 && numericValue <= 10000000) { // Reasonable upper limit
                    this.startCountingAnimation(metricValue, numericValue, suffix);
                }
            }
        }

        startCountingAnimation(element, targetValue, suffix) {
        ...
        addIndividualCardObserver(card) {
            ...
            this.registerCleanupTask(() => cardObserver.disconnect());
        }

        addContinuousAnimations(card) {
            // Add continuous subtle animations
            const continuousAnimation = () => {
                if (card.classList.contains('animate-in')) {
                    ...
                    const breathe = Math.sin(time * 0.5 + parseInt(card.dataset.index || '0')) * 0.02 + 1;
                    card.style.transform = card.style.transform.replace(/scale\([^)]*\)/, '') + ` scale(${breathe})`;

                    // Subtle color shifting for tech badges
                    const techBadges = card.querySelectorAll('.tech-badge-3');
                    techBadges.forEach((badge, badgeIndex) => {
                        const hueShift = Math.sin(time + parseInt(card.dataset.index || '0') + badgeIndex) * 10;
                        badge.style.filter = `hue-rotate(${hueShift}deg)`;
                    });
                }
        ...
        setupCTAButtonInteractions() {
            const ctaButtons3 = document.querySelectorAll('.cta-btn-3');
            ctaButtons3.forEach(button => {
                const clickHandler = (e) => {
                    ...
                    ripple.style.cssText = `
                        ...
                        border-radius: 50%;
                        transform: scale(0);
                        animation: section3Ripple 0.8s ease-out;
                        pointer-events: none;
                        z-index: 10;
                    `;
                    ...
                    // Update custom cursor state
                    this.updateCustomCursor(button, 'click');
                };

                button.addEventListener('click', clickHandler);
                this.registerEventListener(button, 'click', clickHandler);
            });
        }
            ...
            if ('ontouchstart' in window) {
                this.projectCards3.forEach(card => {
                    ...
                    const touchEndHandler = (e) => {
                        ...
                        // If touch duration is short (< 300ms), treat as click
                    ...
                    this.registerEventListener(card, 'touchstart', touchStartHandler);
                    this.registerEventListener(card, 'touchend', touchEndHandler);
                });
            }
        }
        ...
        setupKeyboardNavigation() {
            this.projectCards3.forEach(card => {
                ...
                card.setAttribute('aria-label', 'Project showcase');
                ...
                this.registerEventListener(card, 'keydown', keyDownHandler);
            });
        }
        ...
        setupCustomCursorIntegration() {
            // Integrate with existing custom cursor system
            if (window.customCursor) {
                // Add project card states to cursor
                this.projectCards3.forEach(card => {
                    const mouseEnterHandler = () => {
                        window.customCursor.cursor.classList.add('custom-cursor--project-card');
                    };

                    const mouseLeaveHandler = () => {
                        window.customCursor.cursor.classList.remove('custom-cursor--project-card');
                    };
        ...
        updateCustomCursor(element, state) {
            if (window.customCursor) {
                switch (state) {
                    case 'hover':
                        window.customCursor.cursor.classList.add('custom-cursor--project-hover');
                        break;
                    case 'click':
                        window.customCursor.cursor.classList.add('custom-cursor--project-click');
                        break;
                    case 'normal':
                        window.customCursor.cursor.classList.remove('custom-cursor--project-hover', 'custom-cursor--project-click');
                        break;
                }
            }
        }
        ...
        setupPerformanceOptimizations() {
            ...
            if (performance.memory) {
                ...
                this.checkPerformanceInterval = setInterval(() => {
        ...
        optimizeAnimations() {
            ...
            if (this.animationId) {
                ...
                this.animationId = null;
                this.startOptimizedAnimationLoop();
            }
        }

        startOptimizedAnimationLoop() {
            const animateOptimized = () => {
                const now = performance.now();
                if (now - this.lastAnimationTime > 32) { // ~30fps
        ...
        setupDebugging() {
            ...
            window.debugFeaturedWorkAnimations = () => {
                console.log('=== FEATURED WORK ANIMATIONS DEBUG ===');
                console.log('Is initialized:', this.isInitialized);
                console.log('Section element:', this.featuredWorkSection3);
                console.log('Project cards count:', this.projectCards3.length);
                console.log('CTA element:', this.featuredCTA3);
                console.log('Animation ID:', this.animationId);
                console.log('Event listeners count:', this.eventListeners.length);
                console.log('Cleanup tasks count:', this.cleanupTasks.length);

                return {
                    isInitialized: this.isInitialized,
                    section: this.featuredWorkSection3,
                    projectCards: this.projectCards3,
                    cta: this.featuredCTA3,
                    animationId: this.animationId
                };
            };

            window.testFeaturedWorkAnimations = () => {
                console.log('Testing Featured Work Animations...');

                if (!this.isInitialized) {
                    console.error('FeaturedWorkAnimations not initialized');
                    return false;
                }

                // Test project card animations
                this.projectCards3.forEach((card, index) => {
                    card.classList.add('animate-in');
            ...
            console.log('🔧 Debug functions available: debugFeaturedWorkAnimations(), testFeaturedWorkAnimations()');
        }

        handleInitializationError(error) {
            console.error('FeaturedWorkAnimations initialization failed:', error);

            // Attempt recovery
            setTimeout(() => {
                console.log('Attempting to reinitialize FeaturedWorkAnimations...');
                this.destroy();
                this.init();
            }, 2000);
        }
        ...
        destroy() {
            ...
            if (this.checkPerformanceInterval) {
                ...
                this.checkPerformanceInterval = null;
            }
            ...
            this.isInitialized = false;
            console.log('FeaturedWorkAnimations destroyed');
        }
        ...
        pauseAnimations() {
            ...
            console.log('FeaturedWorkAnimations paused');
        }
        ...
        resumeAnimations() {
            ...
            console.log('FeaturedWorkAnimations resumed');
        }

        getStats() {
            return {
                isInitialized: this.isInitialized,
                projectCardsCount: this.projectCards3.length,
    ...
    // Initialize the Featured Work Animations
    const featuredWorkAnimations = new FeaturedWorkAnimations();

    // Make globally accessible for debugging
    window.featuredWorkAnimations = featuredWorkAnimations;

    // ===== FEATURED WORK SECTION 3 ANIMATIONS =====
    const featuredWorkSection3 = document.querySelector('.featured-work-section-3');
    const projectCards3 = document.querySelectorAll('.project-card-3');
    const featuredCTA3 = document.querySelector('.featured-cta-3');

    if (featuredWorkSection3) {
        // Create intersection observer for section 3 animations
        const featuredWorkObserver3 = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    // Animate project cards with staggered reveal
                    projectCards3.forEach((card, index) => {
                        ...
                        setTimeout(() => {
                            ...
                            // Add interactive hover effects
                            card.addEventListener('mouseenter', function() {
                                this.style.transform += ' rotateY(3deg) rotateX(-1deg)';
                                ...
                                // Store glow reference for removal
                                this.glowEffect3 = glow;
                            });

                            card.addEventListener('mouseleave', function() {
                                this.style.transform = this.style.transform.replace(/ rotateY\(3deg\) rotateX\(-1deg\)/g, '');
                                if (this.glowEffect3) {
                                    this.glowEffect3.style.opacity = '0';
                                    setTimeout(() => {
                                        if (this.glowEffect3 && this.glowEffect3.parentNode) {
                                            this.glowEffect3.remove();
                                        }
                                    }, 400);
                                }
                                this.style.zIndex = '';
                    ...
                    setTimeout(() => {
                        if (featuredCTA3) {
                            const ctaContainer = featuredCTA3.querySelector('.cta-container-3');
                            if (ctaContainer) {
                                ctaContainer.style.opacity = '0';
                                ctaContainer.style.transform = 'translateY(40px) scale(0.95)';
                                ctaContainer.style.transition = 'all 1.5s cubic-bezier(0.16, 1, 0.3, 1)';
                    ...
                    }, projectCards3.length * 250 + 600);

                    // Unobserve after animation to prevent re-triggering
                    featuredWorkObserver3.unobserve(entry.target);
                }
            });
        }, {
            threshold: 0.1,
        ...
        featuredWorkObserver3.observe(featuredWorkSection3);

        // Enhanced mouse tracking for project cards 3
        let mouseX3 = 0, mouseY3 = 0;
        document.addEventListener('mousemove', (e) => {
            mouseX3 = e.clientX;
            mouseY3 = e.clientY;
        });
        ...
        // Continuous animation loop for interactive elements
        function animateFeaturedElements3() {
            const rect = featuredWorkSection3.getBoundingClientRect();
            const isVisible = rect.top < window.innerHeight && rect.bottom > 0;

            if (isVisible) {
                const x = (mouseX3 / window.innerWidth) - 0.5;
                const y = (mouseY3 / window.innerHeight) - 0.5;
                ...
                projectCards3.forEach((card, index) => {
                    if (card.classList.contains('animate-in')) {
                        ...
                        const translateX = (x * speed) + floatX;
                        const translateY = (y * speed) + floatY;
                        const rotateZ = (x * rotateSpeed) + (Math.sin(time * 0.8 + index) * 0.8);
            ...
            requestAnimationFrame(animateFeaturedElements3);
        }

        animateFeaturedElements3();

        // Add click ripple effect to project cards 3
        ...
        // Add feature list animations on hover
        projectCards3.forEach(card => {
            const features = card.querySelectorAll('.project-features-3 li');
        ...
        function addContinuousAnimations() {
            projectCards3.forEach((card, index) => {
                if (card.classList.contains('animate-in')) {
                    ...
                    // Subtle breathing effect
                    const breathe = Math.sin(time * 0.5 + index) * 0.02 + 1;
                    card.style.transform = card.style.transform.replace(/scale\([^)]*\)/, '') + ` scale(${breathe})`;

                    // Subtle color shifting for tech badges
                    const techBadges = card.querySelectorAll('.tech-badge-3');
                    techBadges.forEach((badge, badgeIndex) => {
                        const hueShift = Math.sin(time + index + badgeIndex) * 10;
                        badge.style.filter = `hue-rotate(${hueShift}deg)`;
                    });
                }
            });

            requestAnimationFrame(addContinuousAnimations);
        }

        addContinuousAnimations();
    ...
    console.log('Featured Work Section 3 animations initialized successfully!');
    console.log('Landing page initialized successfully!');

    // --- Stats Section Rolling Counter Animation ---
    const statsSection = document.querySelector('.text-gray-600.body-font');
    const statNumbers = document.querySelectorAll('.stat-number');

    console.log('Stats section found:', !!statsSection);
    console.log('Stat numbers found:', statNumbers.length);
    console.log('GSAP loaded:', typeof gsap !== 'undefined');
    console.log('Stats section visibility:', statsSection ? getComputedStyle(statsSection).visibility : 'N/A');
    console.log('Stats section display:', statsSection ? getComputedStyle(statsSection).display : 'N/A');
    console.log('Stats section opacity:', statsSection ? getComputedStyle(statsSection).opacity : 'N/A');

    if (statsSection && statNumbers.length > 0) {
        // Create intersection observer for stats animation
        const statsObserver = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                console.log('Stats observer entry:', {
                ...
                if (entry.isIntersecting) {
                    console.log('Stats section intersecting, starting animations');
        ...
        function animateStatNumber(statElement) {
            console.log('Animating stat number:', statElement.textContent);
            console.log('GSAP available in animateStatNumber:', typeof gsap !== 'undefined');
            const finalText = statElement.textContent.trim();
            const numberMatch = finalText.match(/^(\d+(?:\.\d+)?)([KMB]?)$/);

            if (numberMatch) {
                let finalNumber = parseFloat(numberMatch[1]);
                const suffix = numberMatch[2];
                const duration = 2; // Animation duration in seconds

                // Convert to actual number
                if (suffix === 'K') finalNumber *= 1000;
                else if (suffix === 'M') finalNumber *= 1000000;
                else if (suffix === 'B') finalNumber *= 1000000000;

                console.log('Final number:', finalNumber, 'Suffix:', suffix);
                ...
                try {
                    gsap.fromTo(statElement, {
                        ...
                        ease: "power2.out",
                        onUpdate: function() {
                            console.log('GSAP onUpdate triggered, current innerText:', this.targets()[0].innerText);
                            // Update the text with formatted number and suffix
                            const currentNumber = parseFloat(this.targets()[0].innerText);
                            const formattedNumber = formatNumber(currentNumber, suffix);
                            this.targets()[0].innerText = formattedNumber;
                        },
                        onComplete: function() {
                            console.log('GSAP animation completed');
                            // Ensure final value is exact
                            this.targets()[0].innerText = finalText;
                        }
                    });
                } catch (error) {
                ...
                console.log('No number match for:', finalText);
            }
        }
        ...
        function formatNumber(num, suffix) {
            if (suffix === 'K') {
                ...
                return (num / 1000000000).toFixed(1) + suffix;
            } else {
...
*/
    ...
    const projectData = {
        'ecommerce': {
            ...
            description: 'A comprehensive digital marketplace transformation that revolutionized online retail through intelligent automation, personalized shopping experiences, and seamless omnichannel integration. This platform combines cutting-edge AI with modern web technologies to create the ultimate shopping destination.',
            images: [
                'assets/specs.jpeg',
                'assets/earring.jpeg',
                'assets/powerpops.jpeg',
                'assets/princles.jpeg',
                'assets/shine.jpeg',
                'assets/sweet_escape.jpeg',
                'assets/water_bottle.jpeg'
            ],
            technologies: ['Next.js', 'React', 'Node.js', 'MongoDB', 'Stripe', 'AWS', 'Redis', 'GraphQL', 'TensorFlow'],
            metrics: [
                { label: 'Monthly Transactions', value: '$5.2M' },
                { label: 'Conversion Rate', value: '6.8%' },
                { label: 'User Retention', value: '89%' },
                { label: 'Load Time', value: '1.2s' },
                { label: 'Mobile Users', value: '78%' }
            ],
            challenge: 'Traditional e-commerce platforms suffered from poor user experience, limited personalization, and inefficient checkout processes. Customers abandoned carts at 68% rate, and businesses struggled with inventory management and customer insights.',
            solution: 'Built a next-generation e-commerce platform featuring AI-powered product recommendations, one-click checkout, real-time inventory tracking, advanced analytics dashboard, and seamless mobile experience. Implemented machine learning algorithms for personalized shopping experiences and automated marketing campaigns.',
            results: 'Achieved 340% increase in conversion rate, reduced cart abandonment by 85%, increased average order value by 156%, and boosted customer lifetime value by 280%. Platform now handles 2M+ monthly transactions with 99.9% uptime.',
            demoUrl: 'https://ecommerce-revolution.Xdot.com',
            beforeAfter: {
                before: 'assets/water_bottle.jpeg',
                after: 'assets/sweet_escape.jpeg'
            }
        },
        'ai-analytics': {
            title: 'AI-Powered Analytics',
            category: 'Business Intelligence',
            description: 'An intelligent analytics platform that transforms raw data into actionable business insights using advanced machine learning algorithms. Features predictive modeling, automated reporting, and real-time dashboard visualizations for data-driven decision making.',
            images: [
                'assets/earring.jpeg',
                'assets/powerpops.jpeg',
                'assets/princles.jpeg',
            ...
            technologies: ['Python', 'TensorFlow', 'Apache Spark', 'React', 'D3.js', 'PostgreSQL', 'Kafka', 'Docker', 'Kubernetes'],
            metrics: [
                { label: 'Data Processing', value: '10TB/day' },
                { label: 'Prediction Accuracy', value: '94.7%' },
                { label: 'Query Response', value: '0.3s avg' },
                { label: 'Cost Reduction', value: '65%' },
                { label: 'ROI Improvement', value: '280%' }
            ],
            challenge: 'Businesses were drowning in data but lacked actionable insights. Manual reporting processes took weeks, predictions were inaccurate, and real-time analytics were non-existent. Decision-makers relied on gut feelings rather than data-driven insights.',
            solution: 'Developed an end-to-end AI analytics platform with automated data ingestion, machine learning models for predictive analytics, natural language query processing, and interactive dashboards. Implemented real-time streaming analytics and automated alert systems for proactive business intelligence.',
            results: 'Reduced reporting time from 2 weeks to 5 minutes, improved prediction accuracy by 95%, increased operational efficiency by 300%, and delivered $12M in cost savings within first year. Platform now processes 10TB of data daily with sub-second query responses.',
            demoUrl: 'https://ai-analytics.Xdot.com',
            beforeAfter: {
                before: 'assets/specs.jpeg',
                after: 'assets/shine.jpeg'
            }
        },
        'mobile-banking': {
            title: 'Next-Gen Banking',
            category: 'FinTech Innovation',
            description: 'A revolutionary mobile banking application that redefines financial services through biometric security, AI-powered financial insights, and seamless digital experiences. Features include instant transfers, intelligent budgeting, and personalized financial advice.',
            images: [
                'assets/powerpops.jpeg',
                'assets/sweet_escape.jpeg',
                'assets/water_bottle.jpeg',
            ...
            technologies: ['React Native', 'Node.js', 'MongoDB', 'Biometric APIs', 'Machine Learning', 'Blockchain', 'AWS Lambda', 'Stripe Connect'],
            metrics: [
                { label: 'Active Users', value: '500K+' },
                { label: 'Transaction Volume', value: '$2.8B' },
                { label: 'App Rating', value: '4.9/5' },
                { label: 'Security Score', value: '99.8%' },
                { label: 'User Retention', value: '92%' }
            ],
            challenge: 'Traditional banking apps were cumbersome, lacked personalization, and failed to provide meaningful financial insights. Customers struggled with complex navigation, poor security perceptions, and limited digital services compared to fintech competitors.',
            solution: 'Created a cutting-edge mobile banking experience with biometric authentication, AI-driven financial insights, voice-activated transactions, and intuitive gesture-based navigation. Implemented advanced security protocols, real-time fraud detection, and personalized financial coaching powered by machine learning.',
            results: 'Achieved 500K+ active users in first 18 months, processed $2.8B in transactions, maintained 99.8% security score, and received 4.9/5 app store rating. Customer satisfaction increased by 340% while reducing operational costs by 60%.',
            demoUrl: 'https://nextgen-banking.Xdot.com',
            beforeAfter: {
                before: 'assets/water_bottle.jpeg',
                after: 'assets/sweet_escape.jpeg'
            }
        ...
        'brand-identity': {
            ...
            description: 'A comprehensive brand transformation project that redefined a legacy corporation\'s visual identity, messaging, and customer experience. From logo redesign to complete brand ecosystem implementation across all touchpoints.',
            images: [
                'assets/princles.jpeg',
                'assets/shine.jpeg',
                'assets/earring.jpeg',
            ...
            technologies: ['Adobe Creative Suite', 'Figma', 'After Effects', 'Blender', 'HTML/CSS', 'JavaScript', 'Motion Design'],
            metrics: [
                ...
                { label: 'Revenue Growth', value: '+95%' }
            ],
            challenge: 'A 50-year-old corporation had outdated branding that failed to resonate with younger demographics. The visual identity was inconsistent across platforms, messaging was unclear, and digital presence was virtually non-existent in the social media age.',
            solution: 'Conducted extensive market research and user interviews to understand target audience preferences. Developed a modern, versatile brand identity with flexible logo system, comprehensive style guide, and multi-platform digital assets. Implemented brand across website, mobile apps, packaging, retail spaces, and marketing materials.',
            results: 'Increased brand recognition by 180%, boosted customer loyalty scores by 250%, gained 45% market share in target demographics, and achieved 95% revenue growth. The new brand identity successfully positioned the company as innovative and customer-centric.',
            demoUrl: 'https://brand-evolution.Xdot.com',
            brandAssets: {
                logo: 'assets/princles.jpeg',
                colors: ['#ec4899', '#8b5cf6', '#06b6d4', '#f59e0b', '#10b981'],
                fonts: ['Inter', 'Poppins', 'Helvetica Neue'],
                guidelines: 'Comprehensive brand guidelines covering logo usage, color palette, typography hierarchy, and application across digital and print media.'
            }
        },
        'saas-dashboard': {
            title: 'SaaS Dashboard',
            category: 'Product Design',
            description: 'An enterprise-grade SaaS dashboard that provides comprehensive business intelligence, workflow automation, and team collaboration tools. Features real-time data visualization, customizable widgets, and seamless third-party integrations.',
            images: [
                'assets/shine.jpeg',
                'assets/princles.jpeg',
                'assets/earring.jpeg',
            ...
            technologies: ['React', 'TypeScript', 'D3.js', 'Node.js', 'PostgreSQL', 'Redis', 'WebSocket', 'Docker', 'Kubernetes'],
            metrics: [
            ...
            challenge: 'Existing business tools were fragmented and lacked integration capabilities. Teams struggled with data silos, manual reporting processes, and poor collaboration features. Decision-makers lacked real-time insights and actionable dashboards.',
            solution: 'Built a unified SaaS platform with modular dashboard system, drag-and-drop widget customization, real-time data streaming, and comprehensive API integrations. Implemented advanced data visualization, automated workflows, and collaborative features with role-based access control.',
            results: 'Attracted 50K+ active users within 2 years, reduced reporting time by 85%, improved team productivity by 300%, and achieved 99.9% uptime. Platform processes over 1 billion data points daily with sub-second response times.',
            demoUrl: 'https://saas-dashboard.Xdot.com',
            beforeAfter: {
                before: 'assets/specs.jpeg',
                after: 'assets/shine.jpeg'
            }
        ...
        'ai-chatbot': {
            ...
            description: 'An intelligent customer support system powered by advanced natural language processing and machine learning. Features include automated ticket resolution, sentiment analysis, proactive customer engagement, and seamless human handoff capabilities.',
            images: [
                'assets/sweet_escape.jpeg',
                'assets/water_bottle.jpeg',
                'assets/powerpops.jpeg',
            ...
            technologies: ['Python', 'TensorFlow', 'NLP', 'React', 'Node.js', 'MongoDB', 'WebSocket', 'AWS', 'Dialogflow'],
            metrics: [
                { label: 'Support Tickets', value: '1M+/month' },
                { label: 'Resolution Rate', value: '87%' },
                { label: 'Response Time', value: '3s avg' },
                { label: 'Customer Satisfaction', value: '4.7/5' },
                { label: 'Cost Reduction', value: '60%' }
            ],
            challenge: 'Customer support teams were overwhelmed with repetitive inquiries, response times averaged 24 hours, and customer satisfaction scores were declining. Manual processes led to inconsistent responses and high operational costs.',
            solution: 'Developed an AI-powered customer support platform with natural language understanding, automated ticket categorization, intelligent response generation, and sentiment analysis. Implemented proactive customer outreach, personalized recommendations, and seamless escalation to human agents when needed.',
            results: 'Handles 1M+ support tickets monthly with 87% automated resolution rate, reduced average response time to 3 seconds, improved customer satisfaction to 4.7/5, and achieved 60% cost reduction. Support team can now focus on complex issues while AI handles routine inquiries.',
            demoUrl: 'https://ai-customer-support.Xdot.com',
            beforeAfter: {
                before: 'assets/water_bottle.jpeg',
                after: 'assets/sweet_escape.jpeg'
            }
    ...
    class ProjectDetailModal {
                    ...
                    <div class="modal-content">
                        ...
                        <div class="project-gallery">
                            <div class="gallery-main">
                                <div class="main-image-container">
                                    <img class="main-image" src="" alt="" loading="lazy">
                        ...
                        <div class="project-details">
                            <div class="details-grid">
                                ...
                                <div class="detail-section challenge-solution">
                                    <div class="challenge">
                                        <h3>The Challenge</h3>
                                        <p></p>
                                    </div>
                        ...
                        <div class="project-actions">
                            <a href="#" class="demo-btn" target="_blank" rel="noopener">
        ...
        setupEventListeners() {
            ...
            // Demo button will be handled when content is populated
        }
        ...
        open(projectId) {
            ...
            gsap.set(this.modal.querySelector('.modal-container'), {
                scale: 0.9,
                y: 50,
                opacity: 0
            });
            ...
            .to(this.modal.querySelector('.modal-container'), {
                scale: 1,
                y: 0,
                opacity: 1,
                duration: 0.5,
            ...
            // Update cursor state
            if (window.customCursor) {
                window.customCursor.cursor.classList.add('modal-active');
            }

            // Dispatch event
            document.dispatchEvent(new CustomEvent('modal:opened', {
                detail: { projectId, project: this.currentProject }
            }));
        }
        ...
        close() {
            ...
            const tl = gsap.timeline({
                onComplete: () => {
                    ...
                    // Update cursor state
                    if (window.customCursor) {
                        window.customCursor.cursor.classList.remove('modal-active');
                    }
            ...
            tl.to(this.modal.querySelector('.modal-container'), {
                scale: 0.9,
                y: 50,
                opacity: 0,
                duration: 0.3,
        ...
        populateContent() {
            ...
            // Header content
            this.modal.querySelector('.project-category').textContent = project.category;
            this.modal.querySelector('.project-title').textContent = project.title;
            ...
            // Technologies
            const techStack = this.modal.querySelector('.tech-stack');
            techStack.innerHTML = project.technologies.map(tech =>
                `<span class="tech-badge">${tech}</span>`
            ).join('');

            // Metrics
            const metricsGrid = this.modal.querySelector('.metrics-grid');
            ...
            // Challenge & Solution
            this.modal.querySelector('.challenge p').textContent = project.challenge;
            this.modal.querySelector('.solution p').textContent = project.solution;

            // Results
            this.modal.querySelector('.results-text').textContent = project.results;

            // Demo button
            const demoBtn = this.modal.querySelector('.demo-btn');
            demoBtn.href = project.demoUrl;
            demoBtn.style.display = project.demoUrl ? 'inline-flex' : 'none';
        }
        ...
        updateGallery() {
            ...
            mainImage.alt = `${project.title} - Image ${this.currentImageIndex + 1}`;
            ...
            nextBtn.style.opacity = this.currentImageIndex < project.images.length - 1 ? '1' : '0.3';
        }
        ...
        animateContentReveals() {
            const elements = this.modal.querySelectorAll('.project-header, .project-gallery, .detail-section, .project-actions');
        ...
        // Before-After Slider functionality (for projects that have it)
        createBeforeAfterSlider(beforeImage, afterImage) {
            const sliderContainer = document.createElement('div');
            sliderContainer.className = 'before-after-slider';
            sliderContainer.innerHTML = `
                <div class="slider-wrapper">
                    <img class="before-image" src="${beforeImage}" alt="Before" loading="lazy">
                    <img class="after-image" src="${afterImage}" alt="After" loading="lazy">
            ...
            this.sliderPosition = 50;
            this.setupSliderEvents(sliderContainer);
        ...
        setupSliderEvents(sliderContainer) {
            ...
            const updateSlider = (clientX) => {
                ...
                afterImage.style.clipPath = `inset(0 ${100 - this.sliderPosition}% 0 0)`;
            };
    ...
    const projectModal = new ProjectDetailModal();

    // Add click handlers for view-project buttons
    document.addEventListener('click', function(e) {
        const viewButton = e.target.closest('.view-project, [data-project-id]');
        if (viewButton) {
            e.preventDefault();
            const projectId = viewButton.dataset.projectId || viewButton.getAttribute('data-project');

            if (projectId && projectData[projectId]) {
                projectModal.open(projectId);
            } else {
                console.error('Project ID not found or invalid:', projectId);
            }
        }
    });
    ...
    console.log('Project Detail Modal system initialized successfully!');

    // --- Mobile Hamburger Menu Functionality ---
    const hamburgerMenu = document.getElementById('hamburgerMenu');
    const navMenu = document.getElementById('navMenu');

    console.log('Hamburger menu elements:', { hamburgerMenu, navMenu });

    if (hamburgerMenu && navMenu) {
        console.log('Setting up hamburger menu event listeners');
        
        // Flag to prevent "click outside" handler from interfering
        let hamburgerJustClicked = false;
        
        hamburgerMenu.addEventListener('click', function(e) {
            e.stopPropagation(); // Prevent event bubbling
            hamburgerJustClicked = true;
            setTimeout(() => { hamburgerJustClicked = false; }, 10);
            
            const isActive = hamburgerMenu.classList.toggle('active');
            navMenu.classList.toggle('active');
            
            console.log('Hamburger menu clicked. Active state:', isActive);
        ...
        // Close menu when clicking Get in Touch button
        if (simpleBtn) {
        ...
        document.addEventListener('click', function(event) {
            // Skip if hamburger was just clicked
            if (hamburgerJustClicked) return;
        ...
        console.warn('Hamburger menu elements not found!', { hamburgerMenu, navMenu });
    }
});
